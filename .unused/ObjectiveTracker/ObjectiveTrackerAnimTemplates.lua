...
BFI_ObjectiveTrackerAnimLineState = EnumUtil.MakeEnum(
    "Adding",
    "Present",
    "Completing",
    "Completed",
...
BFI_ObjectiveTrackerAnimLineMixin = CreateFromMixins(BFI_ObjectiveTrackerLineMixin);

function BFI_ObjectiveTrackerAnimLineMixin:OnGlowAnimFinished()
    if self.state == BFI_ObjectiveTrackerAnimLineState.Completing then
        self.state = BFI_ObjectiveTrackerAnimLineState.Completed;
        self:UpdateModule();
    elseif self.state == BFI_ObjectiveTrackerAnimLineState.Adding then
        self.state = BFI_ObjectiveTrackerAnimLineState.Present;
    end
end

function BFI_ObjectiveTrackerAnimLineMixin:OnFadeOutAnimFinished()
    self.state = BFI_ObjectiveTrackerAnimLineState.Faded;
    local needsUpdate = true;
    local block = self.parentBlock;
    block:ForEachUsedLine(function(line, objectiveKey)
        if line.state == BFI_ObjectiveTrackerAnimLineState.Fading then
...
function BFI_ObjectiveTrackerAnimLineMixin:SetState(desiredState)
    ...
    if desiredState == BFI_ObjectiveTrackerAnimLineState.Present then
        self.Icon:Hide();
    elseif desiredState == BFI_ObjectiveTrackerAnimLineState.Completed then
        if not self.noIcon then
            self.Icon:Show();
        end
    elseif desiredState == BFI_ObjectiveTrackerAnimLineState.Completing then
        if not self.noIcon then
            self.Icon:Show();
            self.CheckAnim:Play();
        end
        self.GlowAnim:Play();
    elseif desiredState == BFI_ObjectiveTrackerAnimLineState.Adding then
        self.GlowAnim:Play();
    elseif desiredState == BFI_ObjectiveTrackerAnimLineState.Fading then
...
function BFI_ObjectiveTrackerAnimLineMixin:SetNoIcon(noIcon)
    self.noIcon = noIcon;
    self.Icon:Hide();
end

function BFI_ObjectiveTrackerAnimLineMixin:OnFree(block)
    if self.state then
        self.state = nil;
        self.noIcon = nil;
...
BFI_ObjectiveTrackerAnimBlockMixin = CreateFromMixins(BFI_ObjectiveTrackerBlockMixin);

function BFI_ObjectiveTrackerAnimBlockMixin:OnLayout()
    if self.parentModule:NeedsFanfare(self.id) then
        self:TryPlayAnim(self.AddAnim);
    end
end

function BFI_ObjectiveTrackerAnimBlockMixin:HasActiveAnim()
    return not not self.activeAnim;
end

function BFI_ObjectiveTrackerAnimBlockMixin:OnAnimFinished()
    if self.activeAnim ~= self.AddAnim then
        if not BFI_ObjectiveTrackerManager:HasRewardsToastForBlock(self) then
...
function BFI_ObjectiveTrackerAnimBlockMixin:OnAnimStopped()
    if self.activeAnim == self.AddAnim then
...
function BFI_ObjectiveTrackerAnimBlockMixin:PlayAddAnimation()
...
function BFI_ObjectiveTrackerAnimBlockMixin:PlayTurnInAnimation()
...
function BFI_ObjectiveTrackerAnimBlockMixin:PlayRemoveAnimation()
...
-- overrides base
function BFI_ObjectiveTrackerAnimBlockMixin:Free()
...
function BFI_ObjectiveTrackerAnimBlockMixin:TryPlayAnim(anim, arg)
...
function BFI_ObjectiveTrackerAnimBlockMixin:SetExtraAddAnimation(anim)
