local settings = {
    ...
    progressBarTemplate = "BFI_BonusTrackerProgressBarTemplate",
    lineTemplate = "BFI_ObjectiveTrackerAnimLineTemplate",
    blockTemplate = "BFI_BonusObjectiveTrackerBlockTemplate",
    -- for this module and those that inherit from it
    showWorldQuests = false,
    questItemButtonSettings = {
        template = "BFI_QuestObjectiveItemButtonTemplate",
        offsetX = 0,
        offsetY = 0,
    },
    findGroupButtonSettings = {
        template = "BFI_QuestObjectiveFindGroupButtonTemplate",
        offsetX = 5,
        offsetY = 2,
    },
...
BFI_BonusObjectiveTrackerMixin = CreateFromMixins(BFI_ObjectiveTrackerModuleMixin, settings);
...
function BFI_BonusObjectiveTrackerMixin:OnFreeBlock(block)
    if block.id < 0 then
...
function BFI_BonusObjectiveTrackerMixin:OnBlockHeaderEnter(block)
    block:TryShowRewardsTooltip();
end

function BFI_BonusObjectiveTrackerMixin:OnBlockHeaderLeave(block)
    GameTooltip:Hide();
    block.hasRewardsTooltip = nil;
end

function BFI_BonusObjectiveTrackerMixin:OnBlockHeaderClick(block, button)
...
function BFI_BonusObjectiveTrackerMixin:OnEvent(event, ...)
...
function BFI_BonusObjectiveTrackerMixin:OnQuestAccepted(questID)
...
function BFI_BonusObjectiveTrackerMixin:OnQuestTurnedIn(questID)
...
function BFI_BonusObjectiveTrackerMixin:OnQuestRemoved(questID)
    local block = self:GetExistingBlock(questID);
    if block then
...
function BFI_BonusObjectiveTrackerMixin:ShowRewardsToast(block, questID)
    local rewards = { };
    -- xp
    ...
    if #rewards > 0 then
        BFI_ObjectiveTrackerManager:ShowRewardsToast(rewards, self, block, headerText, callback);
    else
        self:RemoveBlockFromCache(block);
    end
end

function BFI_BonusObjectiveTrackerMixin:OnShowRewardsToastDone(block)
    self:RemoveBlockFromCache(block);
end

function BFI_BonusObjectiveTrackerMixin:ProcessScenarioBonusObjectives()
                ...
                for criteriaIndex = 1, numCriteria do
                    ...
                    if criteriaInfo then
                        ...
                        if criteriaInfo.completed then
                            ...
                            line = block:AddObjective(criteriaIndex, criteriaString, nil, nil, BFI_OBJECTIVE_DASH_STYLE_HIDE, BFI_OBJECTIVE_TRACKER_COLOR["Complete"]);
                        ...
                        elseif criteriaInfo.failed then
                            stepFinished = false;
                            line = block:AddObjective(criteriaIndex, criteriaString, nil, nil, BFI_OBJECTIVE_DASH_STYLE_HIDE, BFI_OBJECTIVE_TRACKER_COLOR["Failed"]);
                        else
                            stepFinished = false;
                            line = block:AddObjective(criteriaIndex, criteriaString, nil, nil, BFI_OBJECTIVE_DASH_STYLE_HIDE);
                        end
                        -- timer bar
...
function BFI_BonusObjectiveTrackerMixin:SetUpQuestBlock(block, forceShowCompleted)
    local questID = block.id;
        ...
        if text then
            if finished then
                ...
                if not showAsCompleted or existingLine then
                    local line = block:AddObjective(objectiveIndex, text, nil, nil, BFI_OBJECTIVE_DASH_STYLE_HIDE, BFI_OBJECTIVE_TRACKER_COLOR["Complete"]);
                    ...
                    if existingLine and (not line.state or line.state == BFI_ObjectiveTrackerAnimLineState.Present) then
                        ...
                        line:SetState(BFI_ObjectiveTrackerAnimLineState.Completed);
                    end
                end
            else
                local line = block:AddObjective(objectiveIndex, text, nil, nil, BFI_OBJECTIVE_DASH_STYLE_SHOW);
                line.Icon:Hide();
            end
        end
    ...
    if showAsCompleted then
        ...
        block:AddObjective("QuestComplete", completionText, nil, nil, BFI_OBJECTIVE_DASH_STYLE_HIDE, BFI_OBJECTIVE_TRACKER_COLOR["Complete"]);
    end
    if isWorldQuest and not hasAddedTimeLeft then
    ...
    if showAsCompleted then
        block:ForEachUsedLine(function(line, objectiveKey)
            if line.state == BFI_ObjectiveTrackerAnimLineState.Completed then
                line:SetState(BFI_ObjectiveTrackerAnimLineState.Fading);
            end
        end);
    end
end

function BFI_BonusObjectiveTrackerMixin:AddQuest(questID, isTrackedWorldQuest)
    local isWorldQuest = self.showWorldQuests;
...
function BFI_BonusObjectiveTrackerMixin:LayoutContents()
...
BFI_BonusObjectiveTrackerProgressBarMixin = { };

function BFI_BonusObjectiveTrackerProgressBarMixin:OnLoad()
    self.Bar.Icon:SetMask("Interface\\CharacterFrame\\TempPortraitAlphaMask");
end

function BFI_BonusObjectiveTrackerProgressBarMixin:SetValue(percent)
...
function BFI_BonusObjectiveTrackerProgressBarMixin:OnGet(isNew, questID, finished)
...
function BFI_BonusObjectiveTrackerProgressBarMixin:UpdateReward()
    local _, texture;
...
function BFI_BonusObjectiveTrackerProgressBarMixin:PlayFlareAnim(delta)
...
function BFI_BonusObjectiveTrackerProgressBarMixin:OnFree()
    self:ResetAnimations();
end

function BFI_BonusObjectiveTrackerProgressBarMixin:ResetAnimations()
...
BFI_BonusObjectiveBlockMixin = CreateFromMixins(BFI_ObjectiveTrackerQuestPOIBlockMixin);

function BFI_BonusObjectiveBlockMixin:OnEnter()
    self:OnHeaderEnter();
end

function BFI_BonusObjectiveBlockMixin:OnLeave()
    self:OnHeaderLeave();
end

function BFI_BonusObjectiveBlockMixin:OnMouseUp(mouseButton)
    self:OnHeaderClick(mouseButton);
end

function BFI_BonusObjectiveBlockMixin:OnRemoveAnimFinished()
    self.parentModule:ForceRemoveBlock(self);
end

function BFI_BonusObjectiveBlockMixin:TryShowRewardsTooltip()
    local questID;
    if self.id < 0 then
...
BFI_ObjectiveTrackerTopBannerMixin = { };

function BFI_ObjectiveTrackerTopBannerMixin:OnLoad()
...
function BFI_ObjectiveTrackerTopBannerMixin:OnHide()
    TopBannerManager_BannerFinished();
end

function BFI_ObjectiveTrackerTopBannerMixin:GetQuestID()
    return self.questID;
end

function BFI_ObjectiveTrackerTopBannerMixin:DisplayForQuest(questID, module)
...
function BFI_ObjectiveTrackerTopBannerMixin:PlayBanner()
...
-- called by TopBannerManager
function BFI_ObjectiveTrackerTopBannerMixin:StopBanner()
    self.PopAnim:Stop();
    self.SlideAnim:Stop();
    self:Hide();
    self:Finish();
...
function BFI_ObjectiveTrackerTopBannerMixin:OnPopAnimFinished()
    -- offsets for anims
    local container = BFI_ObjectiveTrackerManager:GetContainerForModule(self.module);
...
function BFI_ObjectiveTrackerTopBannerMixin:OnSlideAnimFinished()
    self:Hide();
    self:Finish();
end

function BFI_ObjectiveTrackerTopBannerMixin:Finish()
