local settings = {
    headerText = TRACKER_HEADER_CAMPAIGN_QUESTS,
    events = { "QUEST_LOG_UPDATE", "QUEST_WATCH_LIST_CHANGED", "SUPER_TRACKING_PATH_UPDATED" },
    lineTemplate = "BFI_ObjectiveTrackerAnimLineTemplate",
};

BFI_CampaignQuestObjectiveTrackerMixin = CreateFromMixins(BFI_QuestObjectiveTrackerMixin, settings);

function BFI_CampaignQuestObjectiveTrackerMixin:ShouldDisplayQuest(quest)
    return (quest:GetQuestClassification() == Enum.QuestClassification.Campaign) and not quest:IsDisabledForSession();
end
