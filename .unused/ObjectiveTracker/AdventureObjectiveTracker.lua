...
local settings = {
    ...
    blockTemplate = "BFI_ObjectiveTrackerAnimBlockTemplate",
};

BFI_AdventureObjectiveTrackerMixin = CreateFromMixins(BFI_ObjectiveTrackerModuleMixin, settings);

function BFI_AdventureObjectiveTrackerMixin:InitModule()
    -- POIButtonOwnerTemplate
    self:Init();
end

function BFI_AdventureObjectiveTrackerMixin:OnEvent(event, ...)
...
function BFI_AdventureObjectiveTrackerMixin:OnBlockHeaderClick(block, mouseButton)
...
function BFI_AdventureObjectiveTrackerMixin:OnBlockHeaderEnter(block)
    if block.trackableType == Enum.ContentTrackingType.Appearance then
...
function BFI_AdventureObjectiveTrackerMixin:OnBlockHeaderLeave(block)
...
function BFI_AdventureObjectiveTrackerMixin:GetDebugReportInfo(block)
    return { debugType = "AdventureTracked", trackableType = block.trackableType, id = block.trackableID, };
end

function BFI_AdventureObjectiveTrackerMixin:ClickProfessionTarget(recipeID)
...
function BFI_AdventureObjectiveTrackerMixin:OpenToAppearance(appearanceID)
    TransmogUtil.OpenCollectionToItem(appearanceID);
end

function BFI_AdventureObjectiveTrackerMixin:Untrack(trackableType, id)
    C_ContentTracking.StopTracking(trackableType, id, Enum.ContentTrackingStopType.Manual);
end

function BFI_AdventureObjectiveTrackerMixin:ProcessTrackingEntry(trackableType, trackableID)
    ...
    if targetType then
        ...
        if objectiveText then
            block.objective = block:AddObjective(1, objectiveText, LINE_TYPE_ANIM, true, BFI_OBJECTIVE_DASH_STYLE_SHOW, BFI_OBJECTIVE_TRACKER_COLOR["Normal"]);
        else
            block.objective = block:AddObjective(1, CONTENT_TRACKING_RETRIEVING_INFO, LINE_TYPE_ANIM, nil, BFI_OBJECTIVE_DASH_STYLE_HIDE, BFI_OBJECTIVE_TRACKER_COLOR["Normal"]);
        end
        ...
        if NavigableContentTrackingTargets[targetType] then
            ...
            if objectiveText and (trackingResult ~= Enum.ContentTrackingResult.DataPending) then
                if not block.endLocationUIMap then
                    block:AddObjective(2, CONTENT_TRACKING_LOCATION_UNAVAILABLE, nil, nil, BFI_OBJECTIVE_DASH_STYLE_HIDE, BFI_OBJECTIVE_TRACKER_COLOR["Normal"]);
                else
                    ...
                    if (navigableTrackingResult == Enum.ContentTrackingResult.Failure) or
                        ...
                        block:AddObjective(2, CONTENT_TRACKING_ROUTE_UNAVAILABLE, nil, nil, BFI_OBJECTIVE_DASH_STYLE_HIDE, BFI_OBJECTIVE_TRACKER_COLOR["Normal"]);
                    else
                        ...
                        if (trackableType == superTrackedType) and (trackableID == superTrackedID) then
                            ...
                            if waypointText then
                                ...
                                block:AddObjective(2, formattedText, nil, nil, BFI_OBJECTIVE_DASH_STYLE_SHOW, BFI_OBJECTIVE_TRACKER_COLOR["Normal"]);
                            end
                        end
                    end
                end
        ...
        if BFI_ObjectiveTrackerManager:CanShowPOIs(self) then
...
function BFI_AdventureObjectiveTrackerMixin:OnFreeBlock(block)
...
function BFI_AdventureObjectiveTrackerMixin:EnumerateTrackables(callback)
...
function BFI_AdventureObjectiveTrackerMixin:StopTrackingCollectedItems()
...
function BFI_AdventureObjectiveTrackerMixin:OnTrackableItemCollected(trackableType, trackableID)
    ...
    if block then
        if block.objective then
            ...
            block.objective:SetState(BFI_ObjectiveTrackerAnimLineState.Completing);
        end
    ...
    BFI_ObjectiveTrackerManager:ShowRewardsToast(rewards, self, block, COLLECTED, callback);
...
function BFI_AdventureObjectiveTrackerMixin:OnShowRewardsToastDone(block)
    self:RemoveBlockFromCache(block);
end

function BFI_AdventureObjectiveTrackerMixin:LayoutContents()
    -- POIButtonOwnerTemplate
    self:ResetUsage();
