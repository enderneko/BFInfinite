BFI_ObjectiveTrackerBlockMixin = CreateFromMixins(BFI_ObjectiveTrackerSlidingMixin);

-- Called on frame creation
function BFI_ObjectiveTrackerBlockMixin:Init()
    self.usedLines = { };
...
function BFI_ObjectiveTrackerBlockMixin:Reset()
    self.used = false;
    self.nextBlock = nil;
...
function BFI_ObjectiveTrackerBlockMixin:Free()
    -- free all the lines
...
function BFI_ObjectiveTrackerBlockMixin:OnAddedRegion(region, isManaged)
...
function BFI_ObjectiveTrackerBlockMixin:GetLine(objectiveKey, optTemplate)
    ...
    if not line then
        line = BFI_ObjectiveTrackerManager:AcquireFrame(self, template);
        line:SetParent(self);
        line:Show();        
    end
...
function BFI_ObjectiveTrackerBlockMixin:GetExistingLine(objectiveKey)
    return self.usedLines[objectiveKey];
end

function BFI_ObjectiveTrackerBlockMixin:FreeUnusedLines()
    for objectiveKey, line in pairs(self.usedLines) do
...
function BFI_ObjectiveTrackerBlockMixin:FreeLine(line)
    self.usedLines[line.objectiveKey] = nil;
    BFI_ObjectiveTrackerManager:ReleaseFrame(line);
    line:Hide();
    if line.OnFree then
        line:OnFree(self);
    end
...
function BFI_ObjectiveTrackerBlockMixin:ForEachUsedLine(func)
    for objectiveKey, line in pairs(self.usedLines) do
...
function BFI_ObjectiveTrackerBlockMixin:SetStringText(fontString, text, useFullHeight, colorStyle, useHighlight)
    ...
    colorStyle = colorStyle or BFI_OBJECTIVE_TRACKER_COLOR["Normal"];
...
function BFI_ObjectiveTrackerBlockMixin:SetHeader(text)
    self.HeaderText:SetPoint("RIGHT", self.rightEdgeOffset, 0);
    local height = self:SetStringText(self.HeaderText, text, nil, BFI_OBJECTIVE_TRACKER_COLOR["Header"], self.isHighlighted);
    self.height = height;
end

function BFI_ObjectiveTrackerBlockMixin:AddObjective(objectiveKey, text, template, useFullHeight, dashStyle, colorStyle, adjustForNoText, overrideHeight)
    ...
    if line.Dash then
        if not dashStyle then
            dashStyle = BFI_OBJECTIVE_DASH_STYLE_SHOW;
        end
        if line.dashStyle ~= dashStyle then
            if dashStyle == BFI_OBJECTIVE_DASH_STYLE_SHOW then
                line.Dash:Show();
                line.Dash:SetText(QUEST_DASH);
            elseif dashStyle == BFI_OBJECTIVE_DASH_STYLE_HIDE then
                line.Dash:Hide();
                line.Dash:SetText(QUEST_DASH);
            elseif dashStyle == BFI_OBJECTIVE_DASH_STYLE_HIDE_AND_COLLAPSE then
                line.Dash:Hide();
...
function BFI_ObjectiveTrackerBlockMixin:AddCustomRegion(region, optOffsetX, optOffsetY)
...
function BFI_ObjectiveTrackerBlockMixin:AddTimerBar(duration, startTime)
...
function BFI_ObjectiveTrackerBlockMixin:AddProgressBar(id, lineSpacing)
    local line = self.lastRegion;
    if not line then
        return nil;
    end
...
function BFI_ObjectiveTrackerBlockMixin:OnHeaderClick(mouseButton)
    self.parentModule:OnBlockHeaderClick(self, mouseButton);
end

function BFI_ObjectiveTrackerBlockMixin:OnHeaderEnter()
    self.isHighlighted = true;
    self:UpdateHighlight();
    self.parentModule:OnBlockHeaderEnter(self);
end

function BFI_ObjectiveTrackerBlockMixin:OnHeaderLeave()
    self.isHighlighted = false;
    self:UpdateHighlight(); 
    self.parentModule:OnBlockHeaderLeave(self);
end

function BFI_ObjectiveTrackerBlockMixin:UpdateHighlight()
    local headerColor, dashColor;
    if self.isHighlighted then
        headerColor = BFI_OBJECTIVE_TRACKER_COLOR["HeaderHighlight"];
        dashColor = BFI_OBJECTIVE_TRACKER_COLOR["NormalHighlight"];
    else
        headerColor = BFI_OBJECTIVE_TRACKER_COLOR["Header"];
        dashColor = BFI_OBJECTIVE_TRACKER_COLOR["Normal"];      
    end
...
function BFI_ObjectiveTrackerBlockMixin:AdjustSlideAnchor(offsetY)
    self.HeaderText:SetPoint("TOPLEFT", 0, offsetY);
end

function BFI_ObjectiveTrackerBlockMixin:AdjustRightEdgeOffset(offset)
...
function BFI_ObjectiveTrackerBlockMixin:AddRightEdgeFrame(settings, identifier, ...)
...
function BFI_ObjectiveTrackerBlockMixin:HasActiveAnim()
    -- Can override in derived mixins
    return false;
end

BFI_ObjectiveTrackerBlockHeaderMixin = { };

function BFI_ObjectiveTrackerBlockHeaderMixin:OnLoad()
    self:RegisterForClicks("LeftButtonUp", "RightButtonUp");
end

function BFI_ObjectiveTrackerBlockHeaderMixin:OnClick(mouseButton)
    local block = self:GetParent();
    block:OnHeaderClick(mouseButton);
end

function BFI_ObjectiveTrackerBlockHeaderMixin:OnEnter()
    local block = self:GetParent();
    block:OnHeaderEnter();
end

function BFI_ObjectiveTrackerBlockHeaderMixin:OnLeave()
    local block = self:GetParent();
    block:OnHeaderLeave();
end
