...
BFI_AchievementObjectiveTrackerMixin = CreateFromMixins(BFI_ObjectiveTrackerModuleMixin, settings);

local ARENA_CATEGORY = 165;
local MAX_CRITERIA_PER_ACHIEVEMENT = 5;
local TIMED_CRITERIA = { };

function BFI_AchievementObjectiveTrackerMixin:OnEvent(event, ...)
...
function BFI_AchievementObjectiveTrackerMixin:OnBlockHeaderClick(block, mouseButton)
...
function BFI_AchievementObjectiveTrackerMixin:UntrackAchievement(achievementID)
...
function BFI_AchievementObjectiveTrackerMixin:LayoutContents()
...
function BFI_AchievementObjectiveTrackerMixin:AddAchievement(achievementID, achievementName, description)
    ...
    if numCriteria > 0 then
        ...
        for criteriaIndex = 1, numCriteria do
            ...
            local colorStyle = eligible and BFI_OBJECTIVE_TRACKER_COLOR["Normal"] or BFI_OBJECTIVE_TRACKER_COLOR["Failed"];
            ...
            elseif numShownCriteria == MAX_CRITERIA_PER_ACHIEVEMENT and numCriteria > (MAX_CRITERIA_PER_ACHIEVEMENT + 1) then
                ...
                block:AddObjective("Extra", "...", nil, nil, BFI_OBJECTIVE_DASH_STYLE_HIDE);
                ...
                local line = block:AddObjective(criteriaIndex, criteriaString, nil, nil, BFI_OBJECTIVE_DASH_STYLE_SHOW, colorStyle);
                numShownCriteria = numShownCriteria + 1;
                -- timer bar
        ...
        local colorStyle = (not timerFailed and IsAchievementEligible(achievementID)) and BFI_OBJECTIVE_TRACKER_COLOR["Normal"] or BFI_OBJECTIVE_TRACKER_COLOR["Failed"];
        local line = block:AddObjective(1, description, nil, nil, BFI_OBJECTIVE_DASH_STYLE_SHOW, colorStyle);
        if timerShown then
