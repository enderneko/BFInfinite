...
local function AcknowledgeEmberCourtHelpTip()
    if showingEmberCourtHelpTip then
        ...
        EventRegistry:UnregisterCallback("WorldMapOnShow", BFI_ScenarioObjectiveTracker);
    end
end
...
local function CheckEmberCourtHelpTip(widgetSetID)
    if widgetSetID == EMBER_COURT_TUTORIAL_WIDGET_SET_ID then
        if HelpTip:Show(UIParent, emberCourtMapHelpTipInfo) then
            ...
            EventRegistry:RegisterCallback("WorldMapOnShow", AcknowledgeEmberCourtHelpTip, BFI_ScenarioObjectiveTracker);
        end
    else
...
local settings = {
    ...
    lineSpacing = 12,
    fromBlockOffsetY = -2,
    lineTemplate = "BFI_ObjectiveTrackerAnimLineTemplate",
    progressBarTemplate = "BFI_ScenarioProgressBarTemplate",
    -- for this module
...
BFI_ScenarioObjectiveTrackerMixin = CreateFromMixins(BFI_ObjectiveTrackerModuleMixin, settings);

function BFI_ScenarioObjectiveTrackerMixin:InitModule()
...
local function WidgetsLayoutWithOffset(widgetContainerFrame, sortedWidgets, containerOffset)
    ...
    BFI_ScenarioObjectiveTracker:MarkDirty();
end

function BFI_ScenarioObjectiveTrackerMixin:SetupWidgetContainers()
...
function BFI_ScenarioObjectiveTrackerMixin:OnEvent(event, ...)
...
function BFI_ScenarioObjectiveTrackerMixin:ShouldShowCriteria()
    return self.shouldShowCriteria;
end

function BFI_ScenarioObjectiveTrackerMixin:SetShouldShowCriteria(shouldShow)
...
function BFI_ScenarioObjectiveTrackerMixin:CanUpdate()
...
function BFI_ScenarioObjectiveTrackerMixin:SetHasNewStage(hasNewStage)
    self.hasNewStage = hasNewStage;
end

-- override
function BFI_ScenarioObjectiveTrackerMixin:MarkBlocksUnused()
    for i, block in ipairs(self.FixedBlocks) do
        block.used = false;
    end
...
-- override
function BFI_ScenarioObjectiveTrackerMixin:FreeUnusedBlocks()
    for i, block in ipairs(self.FixedBlocks) do
...
function BFI_ScenarioObjectiveTrackerMixin:LayoutContents()
    ...
    local slidingState = BFI_ObjectiveTrackerSlidingState.None;
    if hasNewStage and not inChallengeMode then
        if not isCollapsed then
            if currentStage == 1 or currentStage == self.slidOutStage then
                slidingState = BFI_ObjectiveTrackerSlidingState.SlideIn;
            else
                if not scenarioCompleted then
                    slidingState = BFI_ObjectiveTrackerSlidingState.SlideOut;
                end
            end
        end
    ...
    if slidingState == BFI_ObjectiveTrackerSlidingState.SlideOut then
    ...
    if slidingState == BFI_ObjectiveTrackerSlidingState.SlideIn and self:IsShown() then
        stageBlock:SetupStageTransition(hasNewStage, scenarioCompleted);
        self:SlideInContents();
    end
end

function BFI_ScenarioObjectiveTrackerMixin:LayoutWidgetBlock(block)
...
function BFI_ScenarioObjectiveTrackerMixin:SlideInContents()
...
function BFI_ScenarioObjectiveTrackerMixin:SetStageBlockModelScenesShown(shown)
    if self.StageBlock.WidgetContainer then
        self.StageBlock.WidgetContainer:SetModelScenesShown(shown);
    end
end

function BFI_ScenarioObjectiveTrackerMixin:SlideOutContents()
...
function BFI_ScenarioObjectiveTrackerMixin:OnEndSlide(slideOut, finished)
...
function BFI_ScenarioObjectiveTrackerMixin:UpdateCriteria(numCriteria)
    ...
    for criteriaIndex = 1, numCriteria do
        ...
        if criteriaInfo then
            ...
            if criteriaInfo.completed then
                ...
                line = objectivesBlock:AddObjective(criteriaIndex, criteriaString, nil, nil, BFI_OBJECTIVE_DASH_STYLE_HIDE, BFI_OBJECTIVE_TRACKER_COLOR["Complete"]);
                line.Icon:Show();
                line.Icon:SetAtlas("ui-questtracker-tracker-check", false);
                if existingLine and (not line.state or line.state == BFI_ObjectiveTrackerAnimLineState.Present) then
                    line:SetState(BFI_ObjectiveTrackerAnimLineState.Completing);
                end
            else
                line = objectivesBlock:AddObjective(criteriaIndex, criteriaString, nil, nil, BFI_OBJECTIVE_DASH_STYLE_HIDE);
                line.Icon:Show();
...
function BFI_ScenarioObjectiveTrackerMixin:AddWeightedProgressObjective(stageDescription)
    if not self:ShouldShowCriteria() then
        return;
    end
...
function BFI_ScenarioObjectiveTrackerMixin:AddSpells(allSpellInfo)
...
function BFI_ScenarioObjectiveTrackerMixin:UpdateSpellCooldowns()
...
BFI_ScenarioObjectiveTrackerStageMixin = { };

function BFI_ScenarioObjectiveTrackerStageMixin:OnEnter()
...
function BFI_ScenarioObjectiveTrackerStageMixin:GetBGAtlases(scenarioType, textureKit)
...
function BFI_ScenarioObjectiveTrackerStageMixin:UpdateStageBlock(scenarioID, scenarioType, widgetSetID, textureKit, flags, currentStage, stageName, numStages)
...
function BFI_ScenarioObjectiveTrackerStageMixin:UpdateFindGroupButton(scenarioID)
...
function BFI_ScenarioObjectiveTrackerStageMixin:ClearWidgetSet()
    self.widgetSetID = nil;
    self:UpdateWidgetRegistration();
end

function BFI_ScenarioObjectiveTrackerStageMixin:UpdateWidgetRegistration()
...
function BFI_ScenarioObjectiveTrackerStageMixin:SetupStageTransition(hasNewStage, scenarioCompleted)
...
BFI_ScenarioTimerMixin = { };

function BFI_ScenarioTimerMixin:OnLoad()
...
function BFI_ScenarioTimerMixin:OnEvent(event, ...)
...
function BFI_ScenarioTimerMixin:StopTimer(timerID)
...
function BFI_ScenarioTimerMixin:CheckTimers(...)
    ...
    for i = 1, select("#", ...) do
        ...
        if type == Enum.WorldElapsedTimerTypes.ChallengeMode then
            ...
            if mapID then
                ...
                BFI_ScenarioObjectiveTracker.ChallengeModeBlock:Activate(timerID, elapsedTime, timeLimit);
                return;
            end
        elseif type == Enum.WorldElapsedTimerTypes.ProvingGround then
            local diffID, currWave, maxWave, duration = C_Scenario.GetProvingGroundsInfo()
            if duration > 0 then
                BFI_ScenarioObjectiveTracker.ProvingGroundsBlock:Activate(timerID, elapsedTime, duration, diffID, currWave, maxWave);
                return;
            end
        end
    end
...
BFI_ScenarioObjectiveTrackerChallengeModeMixin = { };

function BFI_ScenarioObjectiveTrackerChallengeModeMixin:OnLoad()
...
function BFI_ScenarioObjectiveTrackerChallengeModeMixin:OnEvent(event)
    if event == "CHALLENGE_MODE_DEATH_COUNT_UPDATED" then
        self:UpdateDeathCount();
    end
end

function BFI_ScenarioObjectiveTrackerChallengeModeMixin:UpdateTime(elapsedTime)
...
function BFI_ScenarioObjectiveTrackerChallengeModeMixin:IsActive()
    return not not self.timerID;
end

function BFI_ScenarioObjectiveTrackerChallengeModeMixin:Activate(timerID, elapsedTime, timeLimit)
    ...
    BFI_ScenarioObjectiveTracker:ForceExpand();
end

function BFI_ScenarioObjectiveTrackerChallengeModeMixin:UpdateDeathCount()
...
function BFI_ScenarioObjectiveTrackerChallengeModeMixin:SetUpAffixes(affixes)
...
BFI_ScenarioChallengeModeAffixMixin = {};

function BFI_ScenarioChallengeModeAffixMixin:SetUp(affixID)
...
function BFI_ScenarioChallengeModeAffixMixin:OnEnter()
...
BFI_ScenarioObjectiveTrackerProvingGroundsMixin = { };

function BFI_ScenarioObjectiveTrackerProvingGroundsMixin:OnLoad()
...
function BFI_ScenarioObjectiveTrackerProvingGroundsMixin:OnEvent(event, ...)
    local score = ...
    self.Score:SetText(score);
end

function BFI_ScenarioObjectiveTrackerProvingGroundsMixin:IsActive()
    return not not self.timerID;
end

local PROVING_GROUNDS_ENDLESS_INDEX = 4;
function BFI_ScenarioObjectiveTrackerProvingGroundsMixin:Activate(timerID, elapsedTime, duration, medalIndex, currWave, maxWave)
    local statusBar = self.StatusBar;
...
function BFI_ScenarioObjectiveTrackerProvingGroundsMixin:UpdateTime(elapsedTime)
    local statusBar = self.StatusBar;
...
function BFI_ScenarioObjectiveTrackerProvingGroundsMixin:OnAnimFinished()
...
BFI_ScenarioRewardsFrameMixin = { };

function BFI_ScenarioRewardsFrameMixin:OnLoad()
    self:SetScale(0.9);
    self.Anim:SetScript("OnFinished", GenerateClosure(self.OnAnimFinished, self));
end

function BFI_ScenarioRewardsFrameMixin:AddReward(label, texture, font)
...
function BFI_ScenarioRewardsFrameMixin:DisplayRewards(xp, money)
...
function BFI_ScenarioRewardsFrameMixin:OnAnimFinished()
    self:Hide();
end
...
BFI_ScenarioTrackerProgressBarMixin = { };

function BFI_ScenarioTrackerProgressBarMixin:OnGet(isNew, criteriaIndex)
    self.Bar.Icon:Hide();
...
function BFI_ScenarioTrackerProgressBarMixin:SetValue(percentage)
    self.Bar:SetValue(percentage);
    self.Bar.Label:SetFormattedText(PERCENTAGE_STRING, percentage);
    self.percentage = percentage;
end

function BFI_ScenarioTrackerProgressBarMixin:PlayFlareAnim(oldPercentage)
...
BFI_ScenarioSpellButtonMixin = { };

function BFI_ScenarioSpellButtonMixin:OnEnter()
    GameTooltip:SetOwner(self, "ANCHOR_RIGHT");
    GameTooltip:SetSpellByID(self.spellID);
end

function BFI_ScenarioSpellButtonMixin:OnClick()
    CastSpellByID(self.spellID);
end

function BFI_ScenarioSpellButtonMixin:SetSpell(spellInfo)
    self.spellID = spellInfo.spellID;
    self.Icon:SetTexture(spellInfo.spellIcon);
    self:UpdateCooldown();
end
        
function BFI_ScenarioSpellButtonMixin:UpdateCooldown()
...
BFI_ScenarioObjectiveTrackerFindGroupButtonMixin = { };

function BFI_ScenarioObjectiveTrackerFindGroupButtonMixin:OnMouseDown()
...
function BFI_ScenarioObjectiveTrackerFindGroupButtonMixin:OnMouseUp()
    if self:IsEnabled() then
...
function BFI_ScenarioObjectiveTrackerFindGroupButtonMixin:OnEnter()
...
function BFI_ScenarioObjectiveTrackerFindGroupButtonMixin:OnLeave()
    GameTooltip:Hide();
end

function BFI_ScenarioObjectiveTrackerFindGroupButtonMixin:OnClick()
    local shouldShowCreateGroupButton = true;
    LFGListUtil_FindScenarioGroup(self.scenarioID, shouldShowCreateGroupButton);
end

function BFI_ScenarioObjectiveTrackerFindGroupButtonMixin:SetScenarioID(scenarioID)
    self.scenarioID = scenarioID;
end
