...
BFI_ObjectiveTrackerContainerMixin = CreateFromMixins(DirtiableMixin, settings);

function BFI_ObjectiveTrackerContainerMixin:OnLoad()
    local dirtyUpdate = true;
    self:SetDirtyMethod(GenerateClosure(self.Update, self, dirtyUpdate));
end

function BFI_ObjectiveTrackerContainerMixin:OnSizeChanged()
    self:MarkDirty();
end

function BFI_ObjectiveTrackerContainerMixin:OnShow()
    -- UIParentManagedFrameMixin.OnShow(self);
    self:UpdateHeight();
end

function BFI_ObjectiveTrackerContainerMixin:OnAdded(backgroundAlpha)
...
function BFI_ObjectiveTrackerContainerMixin:Init()
    if self.Header then
        self.Header.Text:SetText(self.headerText);
    end
end

function BFI_ObjectiveTrackerContainerMixin:GetAvailableHeight()
    return self:GetHeight() - self.topModulePadding;
end

function BFI_ObjectiveTrackerContainerMixin:Update(dirtyUpdate)
...
function BFI_ObjectiveTrackerContainerMixin:AddModule(module)
...
function BFI_ObjectiveTrackerContainerMixin:RemoveModule(module)
    if self:HasModule(module) then
...
function BFI_ObjectiveTrackerContainerMixin:RemoveAllModules()
    self.modules = {};
    self:MarkDirty();
end

function BFI_ObjectiveTrackerContainerMixin:HasAnyModules()
    return #self.modules > 0;
end

function BFI_ObjectiveTrackerContainerMixin:HasModule(module)
    return tContains(self.modules, module);
end

function BFI_ObjectiveTrackerContainerMixin:GetHeightToModule(targetModule)
...
function BFI_ObjectiveTrackerContainerMixin:SetBackgroundAlpha(alpha)
    self.NineSlice:SetAlpha(alpha);
end

function BFI_ObjectiveTrackerContainerMixin:ToggleCollapsed()
    self:SetCollapsed(not self:IsCollapsed());
end

function BFI_ObjectiveTrackerContainerMixin:SetCollapsed(collapsed)
...
function BFI_ObjectiveTrackerContainerMixin:IsCollapsed()
    return self.isCollapsed;
end

function BFI_ObjectiveTrackerContainerMixin:UpdateHeight()
...
function BFI_ObjectiveTrackerContainerMixin:ForceExpand()
...
function BFI_ObjectiveTrackerContainerMixin:ForEachModule(callback)
    for i, module in ipairs(self.modules) do
        callback(module);
    end
end
...
BFI_ObjectiveTrackerContainerHeaderMixin = {};

function BFI_ObjectiveTrackerContainerHeaderMixin:OnLoad()
    self.MinimizeButton:SetScript("OnClick", GenerateClosure(self.OnToggle, self));
    local collapsed = false;
    self:SetCollapsed(collapsed);
end

function BFI_ObjectiveTrackerContainerHeaderMixin:OnToggle()
...
function BFI_ObjectiveTrackerContainerHeaderMixin:SetCollapsed(collapsed)
