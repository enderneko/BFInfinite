BFI_ObjectiveTrackerModuleState = EnumUtil.MakeEnum(
...
local settings = {
    ...
    -- settings
    blockTemplate = "BFI_ObjectiveTrackerBlockTemplate",
    lineTemplate = "BFI_ObjectiveTrackerLineTemplate",
    progressBarTemplate = "BFI_ObjectiveTrackerProgressBarTemplate",
    ...
    -- dynamic
    state = BFI_ObjectiveTrackerModuleState.Skipped,
...
BFI_ObjectiveTrackerModuleMixin = CreateFromMixins(BFI_ObjectiveTrackerSlidingMixin, settings);

function BFI_ObjectiveTrackerModuleMixin:OnLoad()
...
function BFI_ObjectiveTrackerModuleMixin:OnEvent(event, ...)
    -- override in your mixin
end

function BFI_ObjectiveTrackerModuleMixin:OnHide()
    self:EndSlide();
end

function BFI_ObjectiveTrackerModuleMixin:SetContainer(container)
...
function BFI_ObjectiveTrackerModuleMixin:InitModule()
    -- override in your mixin, called the first time the module is added to a container
end

function BFI_ObjectiveTrackerModuleMixin:MarkDirty()
...
function BFI_ObjectiveTrackerModuleMixin:IsDirty()
    return self.isDirty;
end

function BFI_ObjectiveTrackerModuleMixin:HasContents()
    return self.hasContents;
end

function BFI_ObjectiveTrackerModuleMixin:IsDisplayable()
    return self.state >= BFI_ObjectiveTrackerModuleState.ShownPartially;
end

function BFI_ObjectiveTrackerModuleMixin:IsFullyDisplayable()
    return self.state == BFI_ObjectiveTrackerModuleState.ShownFully;
end

function BFI_ObjectiveTrackerModuleMixin:IsComplete()
    return self.state == BFI_ObjectiveTrackerModuleState.NoObjectives or self:IsFullyDisplayable();
end

function BFI_ObjectiveTrackerModuleMixin:IsTruncated()
    return self.state == BFI_ObjectiveTrackerModuleState.NotShown or self.state == BFI_ObjectiveTrackerModuleState.ShownPartially;
end

function BFI_ObjectiveTrackerModuleMixin:GetContentsHeight()
    return self:IsDisplayable() and self.contentsHeight or 0;
end

function BFI_ObjectiveTrackerModuleMixin:SetHeader(text)
    self.Header.Text:SetText(text);
end

-- returns heightUsed, isTruncated
function BFI_ObjectiveTrackerModuleMixin:Update(availableHeight, dirtyUpdate)
    ...
    if availableHeight > 0 then
        ...
        if self.hasContents then
            if self.hasSkippedBlocks and not self:IsCollapsed() then
                self.state = BFI_ObjectiveTrackerModuleState.ShownPartially;
            else
                self.state = BFI_ObjectiveTrackerModuleState.ShownFully;
            end
        else
            if self.hasTriedBlocks then
                self.state = BFI_ObjectiveTrackerModuleState.NotShown;
            else
                self.state = BFI_ObjectiveTrackerModuleState.NoObjectives;
            end
        end
    end
...
function BFI_ObjectiveTrackerModuleMixin:BeginLayout()
    ...
    self.state = BFI_ObjectiveTrackerModuleState.Skipped;
end

function BFI_ObjectiveTrackerModuleMixin:CanUpdate()
    -- override in your mixin
    return true;
end

function BFI_ObjectiveTrackerModuleMixin:LayoutContents()
    -- override in your mixin
    -- must return soon as a LayoutBlock fails
end

function BFI_ObjectiveTrackerModuleMixin:EndLayout()
...
function BFI_ObjectiveTrackerModuleMixin:HasSkippedBlocks()
    return self.hasSkippedBlocks;
end

function BFI_ObjectiveTrackerModuleMixin:UpdateHeight()
...
function BFI_ObjectiveTrackerModuleMixin:SetHeightModifier(key, height)
...
function BFI_ObjectiveTrackerModuleMixin:ClearHeightModifier(key)
    if self.heightModifiers then
...
function BFI_ObjectiveTrackerModuleMixin:AcquireFrame(template)
    local frame, isNew = BFI_ObjectiveTrackerManager:AcquireFrame(self, template);
    frame:SetParent(self.ContentsFrame);
    return frame, isNew;
end

function BFI_ObjectiveTrackerModuleMixin:GetBlock(id, optTemplate)
...
function BFI_ObjectiveTrackerModuleMixin:EnumerateActiveBlocks(callback)
...
function BFI_ObjectiveTrackerModuleMixin:GetExistingBlock(id, optTemplate)
    local template = optTemplate or self.blockTemplate;
...
function BFI_ObjectiveTrackerModuleMixin:MarkBlocksUnused()
    for template, blocks in pairs(self.usedBlocks) do
        for blockID, block in pairs(blocks) do
            block.used = nil;
        end
...
function BFI_ObjectiveTrackerModuleMixin:FreeUnusedBlocks()
    for template, blocks in pairs(self.usedBlocks) do
        for blockID, block in pairs(blocks) do
...
function BFI_ObjectiveTrackerModuleMixin:FreeBlock(block)
    local fromFreeBlock = true;
    self:RemoveBlockFromCache(block, fromFreeBlock);
    block:Free();
    self.usedBlocks[block.template][block.id] = nil;
    BFI_ObjectiveTrackerManager:ReleaseFrame(block);
    self:OnFreeBlock(block);
end

function BFI_ObjectiveTrackerModuleMixin:OnFreeBlock(block)
    -- override in your mixin
end

function BFI_ObjectiveTrackerModuleMixin:ForceRemoveBlock(block)
    block.used = nil;
    self:FreeBlock(block);
    self:MarkDirty();
end
...
function BFI_ObjectiveTrackerModuleMixin:GetNextBlockAnchoring()
...
function BFI_ObjectiveTrackerModuleMixin:LayoutBlock(block)
...
function BFI_ObjectiveTrackerModuleMixin:AddBlock(block)
...
function BFI_ObjectiveTrackerModuleMixin:CanFitBlock(block)
...
function BFI_ObjectiveTrackerModuleMixin:InternalAddBlock(block)
...
function BFI_ObjectiveTrackerModuleMixin:AnchorBlock(block)
...
function BFI_ObjectiveTrackerModuleMixin:GetLastBlock()
    return self.lastBlock or self.Header;
end

function BFI_ObjectiveTrackerModuleMixin:OnBlockHeaderClick(block, mouseButton)
    -- override in your mixin
end

function BFI_ObjectiveTrackerModuleMixin:OnBlockHeaderEnter(block)
    -- override in your mixin
end

function BFI_ObjectiveTrackerModuleMixin:OnBlockHeaderLeave(block)
    -- override in your mixin
end
...
function BFI_ObjectiveTrackerModuleMixin:ToggleCollapsed()
    self:SetCollapsed(not self:IsCollapsed());
end

function BFI_ObjectiveTrackerModuleMixin:SetCollapsed(collapsed)
...
function BFI_ObjectiveTrackerModuleMixin:IsCollapsed()
    return self.isCollapsed;
end

function BFI_ObjectiveTrackerModuleMixin:GetContextMenuParent()
    return BFI_ObjectiveTrackerManager:GetContainerForModule(self);
end

function BFI_ObjectiveTrackerModuleMixin:GetTimerBar(key)
...
function BFI_ObjectiveTrackerModuleMixin:MarkTimerBarsUnused()
...
function BFI_ObjectiveTrackerModuleMixin:FreeUnusedTimerBars()
    if self.usedTimerBars then
        for key, timerBar in pairs(self.usedTimerBars) do
            if not timerBar.used then
                self.usedTimerBars[key] = nil;
                BFI_ObjectiveTrackerManager:ReleaseFrame(timerBar);
            end
        end
    end
end

function BFI_ObjectiveTrackerModuleMixin:GetProgressBar(key, ...)
...
function BFI_ObjectiveTrackerModuleMixin:MarkProgressBarsUnused()
...
function BFI_ObjectiveTrackerModuleMixin:FreeUnusedProgressBars()
    if self.usedProgressBars then
        for key, progressBar in pairs(self.usedProgressBars) do
            if not progressBar.used then
                ...
                BFI_ObjectiveTrackerManager:ReleaseFrame(progressBar);
            end
        end
    end
end
...
function BFI_ObjectiveTrackerModuleMixin:GetRightEdgeFrame(settings, identifier)
...
function BFI_ObjectiveTrackerModuleMixin:MarkRightEdgeFramesUnused()
...
function BFI_ObjectiveTrackerModuleMixin:FreeUnusedRightEdgeFrames()
    if self.usedRightEdgeFrames then
        for key, frame in pairs(self.usedRightEdgeFrames) do
            if not frame.used then
                self.usedRightEdgeFrames[key] = nil;
                BFI_ObjectiveTrackerManager:ReleaseFrame(frame);
            end
        end
    end
end

function BFI_ObjectiveTrackerModuleMixin:AdjustSlideAnchor(offsetY)
...
function BFI_ObjectiveTrackerModuleMixin:SetNeedsFanfare(key)
    if key then
...
function BFI_ObjectiveTrackerModuleMixin:NeedsFanfare(key)
    return self.fanfares and self.fanfares[key];
end

function BFI_ObjectiveTrackerModuleMixin:ClearFanfares()
    self.fanfares = nil;
end

function BFI_ObjectiveTrackerModuleMixin:ForceExpand()
    if self:IsCollapsed() then
...
function BFI_ObjectiveTrackerModuleMixin:AddBlockToCache(block)
    if not block.cached then
        block.cached = true;
...
function BFI_ObjectiveTrackerModuleMixin:RemoveBlockFromCache(block, fromFreeBlock)
...
function BFI_ObjectiveTrackerModuleMixin:UpdateCachedOrderList()
...
function BFI_ObjectiveTrackerModuleMixin:CheckCachedBlocks(upcomingBlock)
...
function BFI_ObjectiveTrackerModuleMixin:MatchesTag(tag)
    return self:GetTag() == tag;
end

function BFI_ObjectiveTrackerModuleMixin:GetTag()
    return self.tag;
end

function BFI_ObjectiveTrackerModuleMixin:AddTag(tag)
    self.tag = tag;
end
...
BFI_ObjectiveTrackerModuleHeaderMixin = {};

function BFI_ObjectiveTrackerModuleHeaderMixin:OnLoad()
    self.MinimizeButton:SetScript("OnClick", GenerateClosure(self.OnToggle, self));
    local collapsed = false;
    self:SetCollapsed(collapsed);
end

function BFI_ObjectiveTrackerModuleHeaderMixin:PlayAddAnimation()
    self.AddAnim:Restart();
end

function BFI_ObjectiveTrackerModuleHeaderMixin:OnToggle()
...
function BFI_ObjectiveTrackerModuleHeaderMixin:SetCollapsed(collapsed)
