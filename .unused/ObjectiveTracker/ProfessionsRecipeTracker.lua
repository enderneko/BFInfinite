...
BFI_ProfessionsRecipeTrackerMixin = CreateFromMixins(BFI_ObjectiveTrackerModuleMixin, settings);

local IsRecrafting = true;

function BFI_ProfessionsRecipeTrackerMixin:OnEvent(event, ...)
...
function BFI_ProfessionsRecipeTrackerMixin:OnBlockHeaderClick(block, mouseButton)
...
function BFI_ProfessionsRecipeTrackerMixin:LayoutContents()
...
function BFI_ProfessionsRecipeTrackerMixin:AddRecipes(isRecraft)
...
function BFI_ProfessionsRecipeTrackerMixin:AddRecipe(recipeID, isRecraft)
        ...
        if ProfessionsUtil.IsReagentSlotRequired(reagentSlotSchematic) then
            ...
            if name then
                ...
                if reagentSlotSchematic:IsVariableQuantityReagent(reagent) then
                    ...
                    local colorStyle = BFI_OBJECTIVE_TRACKER_COLOR["Normal"];
                    local dashStyle = BFI_OBJECTIVE_DASH_STYLE_SHOW;
                    ...
                    local dashStyle = metQuantity and BFI_OBJECTIVE_DASH_STYLE_HIDE or BFI_OBJECTIVE_DASH_STYLE_SHOW;
                    local colorStyle = BFI_OBJECTIVE_TRACKER_COLOR[metQuantity and "Complete" or "Normal"];
                    local line = block:AddObjective(slotIndex, text, nil, nil, dashStyle, colorStyle);
