BFI_OBJECTIVE_TRACKER_COLOR = {
    ...
    BFI_OBJECTIVE_TRACKER_COLOR["Normal"].reverse = BFI_OBJECTIVE_TRACKER_COLOR["NormalHighlight"];
    BFI_OBJECTIVE_TRACKER_COLOR["NormalHighlight"].reverse = BFI_OBJECTIVE_TRACKER_COLOR["Normal"];
    BFI_OBJECTIVE_TRACKER_COLOR["Failed"].reverse = BFI_OBJECTIVE_TRACKER_COLOR["FailedHighlight"];
    BFI_OBJECTIVE_TRACKER_COLOR["FailedHighlight"].reverse = BFI_OBJECTIVE_TRACKER_COLOR["Failed"];
    BFI_OBJECTIVE_TRACKER_COLOR["Header"].reverse = BFI_OBJECTIVE_TRACKER_COLOR["HeaderHighlight"];
    BFI_OBJECTIVE_TRACKER_COLOR["HeaderHighlight"].reverse = BFI_OBJECTIVE_TRACKER_COLOR["Header"];
    BFI_OBJECTIVE_TRACKER_COLOR["TimeLeft"].reverse = BFI_OBJECTIVE_TRACKER_COLOR["TimeLeftHighlight"];
    BFI_OBJECTIVE_TRACKER_COLOR["TimeLeftHighlight"].reverse = BFI_OBJECTIVE_TRACKER_COLOR["TimeLeft"];

BFI_OBJECTIVE_DASH_STYLE_SHOW = 1;
BFI_OBJECTIVE_DASH_STYLE_HIDE = 2;
BFI_OBJECTIVE_DASH_STYLE_HIDE_AND_COLLAPSE = 3;

-- *****************************************************************************************************
-- ***** QUEST ITEM BUTTON
-- *****************************************************************************************************

BFI_QuestObjectiveItemButtonMixin = { };

function BFI_QuestObjectiveItemButtonMixin:OnLoad()
    self:RegisterForClicks("AnyUp");
end

function BFI_QuestObjectiveItemButtonMixin:OnEvent(event, ...)
...
function BFI_QuestObjectiveItemButtonMixin:OnUpdate(elapsed)
    ...
    if rangeTimer then
        ...
        if rangeTimer <= 0 then
            ...
            if not charges or charges ~= self.charges then
                BFI_QuestObjectiveTracker:MarkDirty();
                return;
            end
...
function BFI_QuestObjectiveItemButtonMixin:OnShow()
    self:RegisterEvent("PLAYER_TARGET_CHANGED");
    self:RegisterEvent("BAG_UPDATE_COOLDOWN");
    self:RegisterEvent("PLAYER_INSIDE_QUEST_BLOB_STATE_CHANGED");
end

function BFI_QuestObjectiveItemButtonMixin:OnHide()
    self:UnregisterEvent("PLAYER_TARGET_CHANGED");
    self:UnregisterEvent("BAG_UPDATE_COOLDOWN");
    self:UnregisterEvent("PLAYER_INSIDE_QUEST_BLOB_STATE_CHANGED");
end

function BFI_QuestObjectiveItemButtonMixin:OnEnter()
    GameTooltip:SetOwner(self, "ANCHOR_RIGHT");
    local questLogIndex = self:GetAttribute("questLogIndex");
    GameTooltip:SetQuestLogSpecialItem(questLogIndex);
end

function BFI_QuestObjectiveItemButtonMixin:OnClick(button)
    local questLogIndex = self:GetAttribute("questLogIndex");
...
function BFI_QuestObjectiveItemButtonMixin:SetUp(questLogIndex)
    local link, item, charges, showItemWhenComplete = GetQuestLogSpecialItemInfo(questLogIndex);
...
function BFI_QuestObjectiveItemButtonMixin:UpdateCooldown()
    local questLogIndex = self:GetAttribute("questLogIndex");
...
function BFI_QuestObjectiveItemButtonMixin:CheckUpdateInsideBlob()
    local questID = self:GetAttribute("questID");
    self:UpdateInsideBlob(questID, C_Minimap.IsInsideQuestBlob(questID));
end

function BFI_QuestObjectiveItemButtonMixin:UpdateInsideBlob(questID, inside)
...
BFI_QuestObjectiveItemGlowAnimMixin = {}

function BFI_QuestObjectiveItemGlowAnimMixin:OnLoop()
...
function BFI_QuestObjectiveItemGlowAnimMixin:BeginPlaying()
...
function BFI_QuestObjectiveItemGlowAnimMixin:StopPlaying()
    self.region:Hide();
...
BFI_QuestObjectiveFindGroupButtonMixin = { };

function BFI_QuestObjectiveFindGroupButtonMixin:SetUp(questID)
    self:SetAttribute("questID", questID);
end

function BFI_QuestObjectiveFindGroupButtonMixin:OnMouseDown()
    if self:IsEnabled() then
        self.Icon:SetPoint("CENTER", self, "CENTER", -2, -1);
    end
end

function BFI_QuestObjectiveFindGroupButtonMixin:OnMouseUp()
    if self:IsEnabled() then
        self.Icon:SetPoint("CENTER", self, "CENTER", -1, 0);
    end
end

function BFI_QuestObjectiveFindGroupButtonMixin:OnEnter()
...
function BFI_QuestObjectiveFindGroupButtonMixin:OnClick()
...
BFI_ObjectiveTrackerRewardsToastMixin = {};

function BFI_ObjectiveTrackerRewardsToastMixin:OnLoad()
...
function BFI_ObjectiveTrackerRewardsToastMixin:ShowRewards(rewards, module, block, headerText, callback)
    ...
    self:ClearAllPoints();
    local container = BFI_ObjectiveTrackerManager:GetContainerForModule(module);
...
function BFI_ObjectiveTrackerRewardsToastMixin:OnAnimateRewardsDone()
    if self.callback then
        self.callback();
        self.callback = nil;
    end
    BFI_ObjectiveTrackerManager:HideRewardsToast(self);
end
...
BFI_ObjectiveTrackerLineMixin = {};

function BFI_ObjectiveTrackerLineMixin:OnLoad()
    -- override in your mixin
end

function BFI_ObjectiveTrackerLineMixin:OnHyperlinkClick(link, text, button)
    SetItemRef(link, text, button);
end

function BFI_ObjectiveTrackerLineMixin:UpdateModule()
...
BFI_ObjectiveTrackerProgressBarMixin = { };

function BFI_ObjectiveTrackerProgressBarMixin:SetPercent(percent)
...
BFI_ObjectiveTrackerTimerBarMixin = { };

function BFI_ObjectiveTrackerTimerBarMixin:OnUpdate(elapsed)
...
function BFI_ObjectiveTrackerTimerBarMixin:GetTextColor(timeRemaining)
...
BFI_ObjectiveTrackerSlidingState = EnumUtil.MakeEnum(
    "None",
    "SlideIn",
    "SlideOut"
);

BFI_ObjectiveTrackerSlidingMixin = { };

function BFI_ObjectiveTrackerSlidingMixin:IsSliding()
...
--[[ slideInfo table layout
    ...
    endDelay        : seconds
--]]
function BFI_ObjectiveTrackerSlidingMixin:Slide(slideInfo)
...
function BFI_ObjectiveTrackerSlidingMixin:OnSlideUpdate(elapsed)
...
function BFI_ObjectiveTrackerSlidingMixin:UpdateSlideProgress(progress)
    local slideInfo = self.slideInfo;
    local delta;
    if slideInfo.travel > 0 then
...
function BFI_ObjectiveTrackerSlidingMixin:EndSlide(finished)
...
function BFI_ObjectiveTrackerSlidingMixin:AdjustSlideAnchor(offsetY)
    -- override in your mixin
end

function BFI_ObjectiveTrackerSlidingMixin:OnEndSlide(slideOut, finished)
    -- override in your mixin
end
