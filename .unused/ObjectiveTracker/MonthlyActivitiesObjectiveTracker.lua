...
BFI_MonthlyActivitiesObjectiveTrackerMixin = CreateFromMixins(BFI_ObjectiveTrackerModuleMixin, settings);

function BFI_MonthlyActivitiesObjectiveTrackerMixin:OnEvent(event, ...)
...
function BFI_MonthlyActivitiesObjectiveTrackerMixin:OnBlockHeaderClick(block, mouseButton)
...
function BFI_MonthlyActivitiesObjectiveTrackerMixin:OpenFrameToActivity(activityID)
    if not EncounterJournal then
        EncounterJournal_LoadUI();
    end
...
function BFI_MonthlyActivitiesObjectiveTrackerMixin:UntrackPerksActivity(activityID)
    C_PerksActivities.RemoveTrackedPerksActivity(activityID);
end

function BFI_MonthlyActivitiesObjectiveTrackerMixin:LayoutContents()
    local trackedActivities = C_PerksActivities.GetTrackedPerksActivities().trackedIDs;
    for i = 1, #trackedActivities do
        local activityID = trackedActivities[i];
...
function BFI_MonthlyActivitiesObjectiveTrackerMixin:AddActivity(activityInfo)
    ...
    for index, requirement in ipairs(requirements) do
        if not requirement.completed then
            ...
            block:AddObjective(index, criteriaString, nil, nil, BFI_OBJECTIVE_DASH_STYLE_HIDE_AND_COLLAPSE, BFI_OBJECTIVE_TRACKER_COLOR["Normal"]);
        end
    end
