local settings = {
    ...
    lineTemplate = "BFI_QuestObjectiveLineTemplate",
    blockTemplate = "BFI_ObjectiveTrackerQuestPOIBlockTemplate",
...
BFI_QuestObjectiveTrackerMixin = CreateFromMixins(BFI_ObjectiveTrackerModuleMixin, settings, BFI_AutoQuestPopupTrackerMixin);

function BFI_QuestObjectiveTrackerMixin:InitModule()
    self:AddTag("quest");
    self:WatchMoney(false);
end

function BFI_QuestObjectiveTrackerMixin:OnEvent(event, ...)
...
function BFI_QuestObjectiveTrackerMixin:OnBlockHeaderClick(block, mouseButton)
...
function BFI_QuestObjectiveTrackerMixin:OnBlockHeaderEnter(block)
    if IsInGroup() then
...
function BFI_QuestObjectiveTrackerMixin:OnBlockHeaderLeave(block)
    GameTooltip:Hide();
end

function BFI_QuestObjectiveTrackerMixin:OnFreeBlock(block)
    block.ItemButton = nil;
end

function BFI_QuestObjectiveTrackerMixin:GetDebugReportInfo(block)
...
function BFI_QuestObjectiveTrackerMixin:BuildQuestWatchInfos()
    local infos = {};
...
function BFI_QuestObjectiveTrackerMixin:EnumQuestWatchData(func)
...
function BFI_QuestObjectiveTrackerMixin:DoQuestObjectives(block, questCompleted, questSequenced, isExistingBlock, useFullHeight)
    local questID = block.id;
    ...
    for objectiveIndex = 1, numObjectives do
        ...
        if text then
            ...
            if questCompleted then
                ...
                if line and line.state ~= BFI_ObjectiveTrackerAnimLineState.Faded then
                    line = block:AddObjective(objectiveIndex, text, nil, useFullHeight, BFI_OBJECTIVE_DASH_STYLE_HIDE, BFI_OBJECTIVE_TRACKER_COLOR["Complete"]);
                    -- don't do anything else if a line is either COMPLETING or FADING, the anims' OnFinished will continue the process
                    if not line.state or line.state == BFI_ObjectiveTrackerAnimLineState.Present then
                ...
                if finished then
                    ...
                    if not questSequenced or not objectiveCompleting then
                        ...
                        if questSequenced and isExistingBlock and not line then
                            ...
                            line:SetState(BFI_ObjectiveTrackerAnimLineState.Adding);
                            ...
                            if line.state == BFI_ObjectiveTrackerAnimLineState.Completed then
                                line:SetState(BFI_ObjectiveTrackerAnimLineState.Present);
                            end
                            if objectiveType == "progressbar" then
                                local progressBar = block:AddProgressBar(questID);
                                progressBar:SetPercent(GetQuestProgressBarPercent(questID));
            ...
            if line then
                if line.state == BFI_ObjectiveTrackerAnimLineState.Completing then
...
function BFI_QuestObjectiveTrackerMixin:UpdateSingle(quest)
        ...
        if not objectiveCompleting then
            if quest.isAutoComplete then
                ...
                if completionText then
                    ...
                    block:AddObjective("QuestComplete", completionText, nil, forceCompletedToUseFullHeight, BFI_OBJECTIVE_DASH_STYLE_HIDE);
                else
                    ...
                    if waypointText ~= nil then
                        ...
                        block:AddObjective("QuestComplete", QUEST_WATCH_QUEST_READY, nil, useFullHeight, BFI_OBJECTIVE_DASH_STYLE_HIDE, BFI_OBJECTIVE_TRACKER_COLOR["Complete"]);
                    end
                end
            end
        end
    elseif questFailed then
        block:AddObjective("Failed", FAILED, nil, useFullHeight, BFI_OBJECTIVE_DASH_STYLE_HIDE, BFI_OBJECTIVE_TRACKER_COLOR["Failed"]);
    else
        if shouldShowWaypoint then
            local waypointText = C_QuestLog.GetNextWaypointText(questID);
            if waypointText ~= nil then
...
function BFI_QuestObjectiveTrackerMixin:WatchMoney(watch)
...
function BFI_QuestObjectiveTrackerMixin:LayoutContents()
...
function BFI_QuestObjectiveTrackerMixin:ShouldDisplayQuest(quest)
...
BFI_QuestObjectiveLineMixin = CreateFromMixins(BFI_ObjectiveTrackerAnimLineMixin);

-- overrides base
function BFI_QuestObjectiveLineMixin:OnGlowAnimFinished()
    if self.state == BFI_ObjectiveTrackerAnimLineState.Completing then
        local questID = self.parentBlock.id;
        if IsQuestSequenced(questID) then
            self:SetState(BFI_ObjectiveTrackerAnimLineState.Fading);
        else
            self.state = BFI_ObjectiveTrackerAnimLineState.Completed;
            self:UpdateModule();
        end
    else
