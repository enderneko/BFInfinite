...
BFI_WorldQuestObjectiveTrackerMixin = CreateFromMixins(BFI_BonusObjectiveTrackerMixin, settings);

function BFI_WorldQuestObjectiveTrackerMixin:InitModule()
    self.entryCount = 0;
end

function BFI_WorldQuestObjectiveTrackerMixin:OnEvent(event, ...)
...
function BFI_WorldQuestObjectiveTrackerMixin:TryAddingExpirationWarningLine(block, questID)
    if QuestUtils_ShouldDisplayExpirationWarning(questID) then
        ...
        local line = block:AddObjective("TimeLeft", text, nil, nil, BFI_OBJECTIVE_DASH_STYLE_HIDE, BFI_OBJECTIVE_TRACKER_COLOR["TimeLeft"], true);
        line.Icon:Hide();
    end
end
...
function BFI_WorldQuestObjectiveTrackerMixin:GetSortedWorldQuests()
    local sortedQuests = {};
...
function BFI_WorldQuestObjectiveTrackerMixin:LayoutContents()
    if self.ticker then
        self.ticker:Cancel();
        self.ticker = nil;
    end
